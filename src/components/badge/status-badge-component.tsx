...
export default function StatusBadge({ status }: StatusBadgeProps) {
  ...
  const statusInfo = statusMap[statusString] || {
    label: statusString,
    color:
      defaultGroup[statusString[0] as keyof typeof defaultGroup]?.color ||
      "bg-gray-100 text-gray-600",
    emoji:
      defaultGroup[statusString[0] as keyof typeof defaultGroup]?.emoji || "",
  };

  return (
    <span
      className={`px-3 py-1 rounded-md text-xs font-medium inline-flex items-center gap-1 ${statusInfo.color}`}
    >
      {statusString}{" "}
