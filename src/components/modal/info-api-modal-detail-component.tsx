...
const ModalAPIDetail = (row: any) => (
    <div className="space-y-6 text-sm text-gray-700">
        ...
        <div className="flex items-center flex-wrap gap-4">
            <div>
                <h3 className="text-xs font-semibold text-gray-500 uppercase mb-1">Method</h3>
          ...
          ${row.method === 'DELETE' ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-600'}
        `}>
          {row.method}
        </span>
            </div>
          ...
          ${row.status.startsWith('5') ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-700'}
        `}>
          {row.status} {row.status.startsWith('2') ? '✅' : row.status.startsWith('5') ? '❌' : '⚠️'}
        </span>
            </div>
        </div>
        ...
        {/* URL */}
        <div>
            <h3 className="text-xs font-semibold text-gray-500 uppercase mb-1">URL</h3>
            <p className="break-all text-sm font-medium text-gray-800 bg-gray-50 p-2 rounded-md">{row.url}</p>
        </div>

        {/* Headers */}
        <div>
            <h3 className="text-xs font-semibold text-gray-500 uppercase mb-1">Headers</h3>
        ...
        {row.request?.body?.raw && (
            <div>
                <h3 className="text-xs font-semibold text-gray-500 uppercase mb-1">Request Body</h3>
        ...
        {/* Response Body */}
        <div>
            <h3 className="text-xs font-semibold text-gray-500 uppercase mb-1">Response Body</h3>
            <pre className="bg-gray-100 p-3 rounded-md whitespace-pre-wrap text-xs text-gray-800">
        {(() => {
            try {
                const decoded = new TextDecoder().decode(new Uint8Array(row.response.stream.data));
        ...
        {row.assertions?.length > 0 && (
            <div>
                <h3 className="text-xs font-semibold text-gray-500 uppercase mb-2">Assertions</h3>
                        ...
                        {row.assertions.map((assert: any, index: number) => (
                            <tr key={index} className="border-b">
                                ...
                                <td className="px-3 py-2 font-medium">
                                    {assert.error ? (
                                        <span className="text-red-600">❌ {assert.error.message}</span>
                                    ) : (
