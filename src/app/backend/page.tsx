...
export default function DashboardPage() {
    ...
    function mapExecutionToRowData(executions: any[]) {
         const result = executions.map((item) => ({
            ...
            url: `${item.request.url.protocol}://${item.request.url.host.join('.')}/${item.request.url.path.join('/')}`,
                ...
                {(props) => {
                    ...
                    return (
                        <>
                            ...
                            <td className="p-4 font-medium text-sm text-gray-900">{row?.name}</td>
                            <td className="p-4"><MethodBadge method={row?.method} /></td>
                            <td className="p-4 text-sm text-gray-700 break-words">{row?.url}</td>
                            <td className="p-4"><StatusBadge status={row?.code} /></td>
    ...
    return (
        <DashboardLayout>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 mt-6 w-full">
                ...
                {/* Card ธรรมดา */}
                {selectedDetail && (
                    <MinimalModal title={`🧪 ${selectedDetail.name}`} onClose={() => setSelectedDetail(null)}>
