...
export default function Page() {
  ...
  const [form, setForm] = useState<CancelSalesState["draftValues"]>({
    SchoolID: "",
    sID: "",
    sID2: "",
    sSellID: "",
  ...
  const handleSubmitForm = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const { SchoolID, sID, sID2, sSellID } = form;
    if (
      SchoolID.length < 1 ||
      sID.length < 1 ||
      sID2.length < 1 ||
