...
import { RequestLogin } from "@/stores/type";

// สร้าง async action สำหรับส่งข้อมูลผู้ใช้ไปยัง API
const API_ENDPOINT = `/api/Login`;
export const CallAPI = createAsyncThunk(
  API_METHOD.GET + API_ENDPOINT,
  async (request: RequestLogin["draftValues"]) => {
    const PARAMETER = `?user=${request?.username}&pass=${request?.password}&schoolid=${request?.school_id}`;
